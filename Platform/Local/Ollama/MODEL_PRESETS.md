...
대사 중심이면 `character-voice-bible`, 장면/챕터/원고 작성이면 `novel-writing`, 장편 구조면 `longform-story-design`, 문제 진단이면 `series-qa`, 명시된 `projects/<series-slug>/` 런타임 진행이면 `series-completion-loop`를 내부적으로 선택한다.
...
대사 문제는 `character-voice-bible`, 산문 문제는 `novel-writing`, 장편 설계는 `longform-story-design`, 진단은 `series-qa`, 명시된 런타임 완주 진행은 `series-completion-loop`로 내부 라우팅한다.
...
대사면 `character-voice-bible`, 산문이면 `novel-writing`, 구조면 `longform-story-design`, 진단이면 `series-qa`, 명시된 런타임 진행이면 `series-completion-loop`를 내부적으로 고른다.
