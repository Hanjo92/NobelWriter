# OpenAI / Codex

이 폴더는 Codex 환경에서 이 스킬 세트를 적용할 때 사용하는 플랫폼 전용 파일을 모아두는 자리입니다.

## 목적

- Codex에서 필요한 운영 파일을 분리해서 관리
- 공용 스킬 본문과 플랫폼별 설정을 분리
- 다른 플랫폼과 비교하면서 교체 적용 가능하게 유지

## Codex에서 실제 사용 방법

이 패키지는 별도 `nobelwriter` CLI를 제공하지 않습니다.
Codex가 워크스페이스의 `AGENTS.md`와 `skills/*/agents/openai.yaml`을 읽고 스킬을 자동 호출하는 구조입니다.

즉, 사용 순서는 다음과 같습니다.

1. 이 저장소를 Codex 워크스페이스로 연다.
2. 루트 `AGENTS.md`가 Codex용인지 확인한다.
3. 각 스킬 폴더의 `agents/openai.yaml`이 적용되어 있는지 확인한다.
4. Codex 채팅에 자연어로 집필 요청을 입력한다.

예시:

```text
로맨스 판타지 1화 원고 써줘. 여주 1인칭, 계약 결혼 직전 장면으로.
```

```text
아래 초고를 개고해서 원고로 정리해줘.
```

```text
이 웹소설 기획안을 150~200화 기준으로 재설계해줘.
```

Codex 환경에서는 사용자가 스킬 호출용 CLI 명령을 직접 실행하는 대신, 자연어 요청을 보내면 운영 파일이 적절한 스킬을 라우팅하는 방식으로 쓰는 것이 기본입니다.

## 포함된 적용 파일

이 폴더에는 현재 Codex 적용에 필요한 운영 파일 사본이 들어 있습니다.

- `AGENTS.md`
- `skills/character-voice-bible/agents/openai.yaml`
- `skills/longform-story-design/agents/openai.yaml`
- `skills/novel-writing/agents/openai.yaml`
- `skills/series-completion-loop/agents/openai.yaml`
- `skills/series-qa/agents/openai.yaml`

이 파일들은 루트 워크스페이스에서 사용하는 Codex용 운영 파일을 플랫폼 패키지로 따로 모아둔 것입니다.

## 적용 방식

권장 흐름:

1. 이 폴더의 `AGENTS.md`를 워크스페이스 루트의 `AGENTS.md`로 복사하거나 교체한다.
2. 이 폴더의 `skills/*/agents/openai.yaml` 파일들을 대상 워크스페이스의 같은 경로에 복사하거나 교체한다.
3. `skills/` 본문과 `skills/*/references/`가 대상 워크스페이스에 함께 존재하는지 확인한다.
4. 적용 후 스킬 호출, 라우팅, 파일 저장 규칙이 정상 동작하는지 확인한다.

## 플랫폼 전환 가이드

다른 플랫폼에서 Codex로 전환할 때 처리해야 하는 파일 목록입니다.

### Google Antigravity → Codex

| 작업 | 대상 경로 |
|------|-----------|
| **교체** | `AGENTS.md` → 이 폴더의 `AGENTS.md`로 덮어쓴다 |
| **교체** | `skills/*/agents/antigravity.yaml` → 각각 `openai.yaml`로 교체한다 |
| **삭제** | `skills/*/agents/antigravity.yaml` 파일이 남아 있으면 제거한다 |

### Anthropic Claude → Codex

| 작업 | 대상 경로 |
|------|-----------|
| **교체** | 루트 `CLAUDE.md` → 이 폴더의 `AGENTS.md`로 교체한다 (`CLAUDE.md`는 삭제) |
| **추가** | `skills/*/agents/openai.yaml` 파일들을 각 스킬 폴더에 추가한다 |
| **삭제** | `.claude/commands/` 폴더 전체를 삭제한다 (Codex에서는 사용 안 함) |
| **삭제** | `skills/*/commands/*.md` 파일들을 삭제한다 |

## 체크리스트

- Codex에서 `AGENTS.md`를 읽는 구조인지
- 스킬 라우팅 규칙이 현재 저장소 구조와 맞는지
- `skills/` 경로 기준이 상대경로로 유지되는지
- `drafts/초고`, `drafts/개고`, `drafts/원고` 저장 규칙이 유지되는지

## 메모

이 README는 Codex용 플랫폼 파일을 정리하기 위한 시작점입니다.  
실제 적용 파일이 생기면 아래 섹션을 추가해 구체적으로 기록합니다.

### 적용 파일 목록

- `AGENTS.md`
- `skills/character-voice-bible/agents/openai.yaml`
- `skills/longform-story-design/agents/openai.yaml`
- `skills/novel-writing/agents/openai.yaml`
- `skills/series-completion-loop/agents/openai.yaml`
- `skills/series-qa/agents/openai.yaml`

### 적용 순서

1. `AGENTS.md` 적용
2. 5개 `agents/openai.yaml` 적용
3. `skills/`와 `drafts/` 구조 확인
4. Codex에서 실제 호출 테스트

### 주의사항

- 이 폴더는 Codex 운영 파일만 담습니다.
- `SKILL.md`와 `skills/*/references/`는 공용 자산이므로 기본적으로 루트 `skills/`를 사용합니다.
- 루트 파일이 변경되면 이 패키지 사본도 함께 갱신해야 합니다.
